...
async def emp_add(request: Request) :
    ed = EmpDao()
    emps = ed.selects()
    return templates.TemplateResponse("emp_add.html", {"request": request, "emps" : emps})
...
# @app.get("/emp_delete_act", response_class=HTMLResponse)
# async def emp_delete_act(request: Request, e_id : str):
#
#     ed = EmpDao()
#     cnt = ed.delete(e_id)
#     return templates.TemplateResponse("emp_delete_act.html", {"request": request, "cnt" : cnt})
